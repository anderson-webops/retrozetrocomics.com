...
export function readServerConfig(source: NodeJS.ProcessEnv = process.env) {
	...
	if (!/^[1-9]\d*$/.test(rawPort)) {
		throw new RuntimeConfigurationError("PORT must be an integer from 1 through 65535");
	}
	...
	if (net.isIP(host) === 0 && host !== "localhost") {
		throw new RuntimeConfigurationError("HOST must be localhost or a literal IP address");
	}
	if (nodeEnvironment === "production" && !isLoopbackIp(host)) {
		throw new RuntimeConfigurationError(
			"Production HOST must be a literal loopback address"
		);
	}
