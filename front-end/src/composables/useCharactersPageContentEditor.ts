import type { CharacterBoardProfile, CharacterBoardWorldEntry, CharactersPageContent } from "@/types/site";
...
function isBlankWorldEntry(entry: CharacterBoardWorldEntry) {
	return (
		[entry.body, entry.label, entry.title].every(value => !(value || "").trim()) &&
		(entry.facts || []).every(fact => !(fact.label || "").trim() && !(fact.value || "").trim())
	);
}
...
export function useCharactersPageContentEditor() {
	...
	async function persist(mutate: (nextContent: CharactersPageContent) => void) {
		saving.value = true;
		error.value = "";
		...
		try {
			...
			error.value =
				saveError?.response?.data?.message || saveError?.message || "Unable to save the character board.";
			throw saveError;
		} finally {
			saving.value = false;
		}
	...
	function discardCharacterDraft(characterId: string) {
		...
		const target = content.value.characters.find(character => character.id === characterId);
		if (!target || !isBlankCharacter(target)) {
			return;
		}

		const nextContent = cloneCharactersPageContent(content.value);
		nextContent.characters = nextContent.characters.filter(character => character.id !== characterId);
		apply(nextContent);
	}

	function saveCharacter(character: CharacterBoardProfile) {
		return persist(nextContent => {
			const index = nextContent.characters.findIndex(entry => entry.id === character.id);
			if (index >= 0) {
	...
	function discardWorldEntryDraft(entryId: string) {
		...
		const target = content.value.worldEntries.find(entry => entry.id === entryId);
		if (!target || !isBlankWorldEntry(target)) {
			return;
		}

		const nextContent = cloneCharactersPageContent(content.value);
		nextContent.worldEntries = nextContent.worldEntries.filter(entry => entry.id !== entryId);
		apply(nextContent);
	}

	function saveWorldEntry(entry: CharacterBoardWorldEntry) {
		return persist(nextContent => {
			const index = nextContent.worldEntries.findIndex(currentEntry => currentEntry.id === entry.id);
			if (index >= 0) {
	...
	function removeWorldEntry(entryId: string) {
		if (content.value.worldEntries.length <= 1) {
			error.value = "At least one world entry must remain.";
			return Promise.reject(new Error(error.value));
		}
